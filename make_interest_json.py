...
# 1. 설정 (폴더명 확인 필수!)
image_dir = 'images/result_interest'  # 사진이 들어있는 폴더
json_output = 'data/interest.json'    # 만들어질 결과 파일
...
if not os.path.exists(image_dir):
    ...
    for filename in files:
        ...
        interest_list.append({
            ...
            "date": date_str,
            "category": "interest",
            "title": "설악산, 한편의 시를 위한 길"
        })

    # 3. JSON 저장 (이게 630장 전체를 만듭니다)
    os.makedirs('data', exist_ok=True)
    with open(json_output, 'w', encoding='utf-8') as f:
        json.dump(interest_list, f, ensure_ascii=False, indent=2)

    print(f"✅ 성공! {len(interest_list)}장의 사진 정보를 {json_output}에 담았습니다.")
